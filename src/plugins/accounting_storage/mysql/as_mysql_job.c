...
no_rollup_change:
	...
	if (!job_ptr->db_index) {
		...
		query = xstrdup_printf(
			...
			"het_job_id, emissions_start, het_job_offset, "
	...
	printf("*****as_mysql_job.c.  emissions_start*********");
	printf("%d",job_ptr->emissions_start);
		xstrfmtcat(query,
			   ...
			   job_ptr->het_job_id, job_ptr->emissions_start,het_job_offset,
